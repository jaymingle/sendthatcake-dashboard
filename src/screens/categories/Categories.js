...
class Categories extends Component {
    ...
    renderHeader=() =>{
        return( <tr>
            {["Category Name", "Parent", "Vendor","Descriptions", "Created By","Status", "Actions"].map((head)=><th>{head}</th>)}
    ...
    renderData=()=>{
       ...
       return this.props.prodcat.map(d =>{
        ...
        {
        d.status == "disabled" || d.status == "approved" || d.status =="pending_approval" || d.status =="delcined"
        ?
        <div>
        <a className="btn btn-primary btn-sm dropdown-toggle d-flex align-items-center" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
            <i className="la la-angle-down mr-0"></i>
        ...
        <div className="dropdown-menu" x-placement="top-start" style={{position: 'absolute', willChange: 'transform', top: '0px', left: '0px', transform: 'translate3d(91px, -145px, 0px)'}}>
            ...
            ?
            <div>
            <a className="dropdown-item" onClick={(e)=>{this.changeStatus(d,"approved"); e.preventDefault()}} >Approve</a>
            <a className="dropdown-item" onClick={(e)=>{this.changeStatus(d,"declined"); e.preventDefault()}} >Decline</a>
            </div>
            :
            null
            }
            {d.status == "declined"
            ?
            <a className="dropdown-item" onClick={(e)=>{this.changeStatus(d,"approved"); e.preventDefault()}} >Set To Approve</a>
            :
            null
            }
            <div className="dropdown-divider"></div>
