...
class Products extends Component{
  componentDidMount(){
    ...
    renderHeader=() =>{
      return( <tr>
          {["Name", "Descriptions", "Price", "Store Name", "Status", "Actions"].map((head)=><th>{head}</th>)}
  ...
  renderData=()=>{
     ...
     return this.props.products?.map(p =>{
        ...
        {
        p.status == "disabled" || p.status == "approved" || p.status =="pending_approval"
        ?
        <div>
        ...
        <div className="dropdown-menu" x-placement="top-start" style={{position: 'absolute', willChange: 'transform', top: '0px', left: '0px', transform: 'translate3d(91px, -145px, 0px)'}}>
            ...
            <div>
            <a className="dropdown-item" onClick={(e)=>{this.changeStatus(p,"approved"); e.preventDefault()}} >Approve</a>
            <a className="dropdown-item" onClick={(e)=>{this.changeStatus(p,"declined"); e.preventDefault()}} >Decline</a>
            </div>
            :
            null
            }
        ...
        null
              ...
              <td className="td-actions">
                  <a href="#"><i className="la la-edit edit"></i></a>
                  <Link to="" title="Delete Product" onClick={(e)=>{this.props.deleteProduct(p.id); e.preventDefault()}}><i className="la la-close delete"></i></Link>
              </td>
          </tr>)
      })
  ...
  changeStatus = (cat, status)=>{
    ...
    Object.keys(p).forEach(obj => data.append(''+obj, p[''+obj]))
    // data.append("status",status)
    this.props.updateProduct(p.id, data)
}
  ...
  const mapStateToProps = (state) => {
    return {
        ...
        // category: state.productCategory.prodCat,
